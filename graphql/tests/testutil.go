...
func SetupTestEnv(t *testing.T) *TestEnv {
	...
	err = db.AutoMigrate(&models.Season{}, &models.Category{}, &models.Game{}, &models.Clue{})
	require.NoError(t, err)
	env.db = db
	// Seed Data
	env.SeedData(t)
	...
	os.Setenv("PORT", "4000")
	gqlsrv := handler.NewDefaultServer(gqlserver.NewExecutableSchema(gqlserver.Config{
		Resolvers: &resolvers.Resolver{
			DB: db,
		},
	}))
	require.NotNil(t, gqlsrv)
	env.gqlserver = gqlsrv
	env.httpserver = httptest.NewServer(http.HandlerFunc(gqlsrv.ServeHTTP))
